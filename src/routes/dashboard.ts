...
import { getTeacherAllowedClassIds, requireRoles, requireSchoolScope } from "../utils/authz";
      ...
      try {
        ...
        const perSchool = await Promise.all(
          schools.map(async (school) => {
            ...
            const [attendanceStats, currentlyInSchoolCount, weeklyAttendance] = await Promise.all([
              ...
              prisma.dailyAttendance.count({
                where: { schoolId: school.id, date: today, currentlyInSchool: true },
              }),
              prisma.dailyAttendance.groupBy({
                by: ['date', 'status'],
                where: {
        ...
        const endKey = sortedDateKeys[sortedDateKeys.length - 1] || getDateOnlyInZone(now, "UTC").toISOString().split('T')[0];
        ...
        const weeklyStats = weekDates.map((date) => {
          ...
          return {
            date: dateKey,
            dayName: ["Ya", "Du", "Se", "Ch", "Pa", "Ju", "Sh"][date.getUTCDay()],
    ...
    async (request: any, reply) => {
      try {
        ...
        const allowedClassIds =
          user.role === 'TEACHER' ? await getTeacherAllowedClassIds(user.sub) : [];
        if (user.role === 'TEACHER' && classId && !allowedClassIds.includes(classId)) {
        ...
        const classScopeFilter: any = classId
          ? { classId }
          : user.role === 'TEACHER'
            ? { classId: { in: allowedClassIds.length ? allowedClassIds : ['__none__'] } }
            : {};

        const studentFilter: any = { schoolId, isActive: true, ...classScopeFilter };
        ...
        const attendanceStudentFilter =
          classId
            ? { student: { classId } }
            : user.role === 'TEACHER'
              ? { student: { classId: { in: allowedClassIds.length ? allowedClassIds : ['__none__'] } } }
              : {};
        ...
        const classWhere: any = {
          ...
          ...(classId
            ? { id: classId }
            : user.role === 'TEACHER'
              ? { id: { in: allowedClassIds.length ? allowedClassIds : ['__none__'] } }
              : {}),
        };
